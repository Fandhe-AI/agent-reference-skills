...
- `nvidia-cuda` は CUDA プログラミングスタック（CUDA C++/Python・PTX ISA・Blackwell チューニング・CUTLASS/CuTe）を担当し、`dgx-spark`（GB10 実機仕様・DGX OS・運用 playbook）・`nvidia-sync`（リモート接続アプリ）とは検索キーワードで棲み分ける。`description` は「NVIDIA CUDA」始まりで、`GEMM` / `kernel` が `amd-rocm` と衝突するため CUDA / PTX / CUTLASS 主導の語を優先する。参照本文は英語（docs.nvidia.com が英語のため）
