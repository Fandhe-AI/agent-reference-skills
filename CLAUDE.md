...
- `drizzle` は TypeScript ORM drizzle-orm / CLI drizzle-kit **1.0.0-rc.5** を 1 スキルで収録する（出典 `orm.drizzle.team/docs/`、原本 `github.com/drizzle-team/drizzle-orm-docs` の `src/content/docs/**/*.mdx`）。公式 docs は v1 のみを記述し npm `latest` 0.45.2 用の一次情報源が無いため RC を pin し、`scripts/install.md` 冒頭に `@rc` 必須と無印 install の v0 混入警告を置く。取得は `raw.githubusercontent.com/.../main/src/content/docs/pg/<page>.mdx` を正典とする（一部ページは docs 直下や他方言ディレクトリにのみ存在するため各ページ `## Notes` に転記元を記録）。WebFetch の一括取得は要約器がパラフレーズするため禁止し、narrow verbatim（`~~~` フェンス囲み）で取得する。`.mdx` アンラップ規則として `@mdx` import の削除、`<Npm>` → `npm i`、`<Npx>` → `npx`、CodeTabs / Section / Callout は中身のみ抽出する。`source:` は `/docs/<page>` とし、pg プレフィックス `/docs/pg/*` は**404**、他方言 `/docs/{mysql,sqlite,mssql,cockroach,singlestore}/*` は 200（実測）、column-types は `/docs/column-types/<dialect>` を使う。pg が正典であることと方言差は per-page `## Notes`（未確認の方言差は書かない）。12 カテゴリ（getting-started / schema / column-types / connect / queries / relational-queries / drizzle-kit / advanced / validations / extensions / migration / seeding）で構成し、README は H1 + 索引表のみ（`mssql` 先例、バージョン行は SKILL.md 本文のみに明記）。migration は `references/migration/v0-v1-changes.md` に加え影響ページへ `Migration (v0 → v1): ...` 形式の `## Notes` を置く（`meta/_journal.json` 廃止・DDL snapshot 化、`drizzle-kit drop` 削除、mssql-core / cockroach-core 新設、relations v2 `defineRelations` 等。`fastify` の `guides/migration-v5.md` と同運用）。`description` は「Drizzle ORM (TypeScript ORM, drizzle-orm / drizzle-kit) の」で始め、裸の select / insert / query / transaction / schema / migrate / cache を lead word にしない。距離線は `mssql`（双方向、`mssql` の description にも追記）・`zod`（双方向、drizzle-zod）・`supabase`（rls / connect-supabase）・`upstash`（advanced/cache）に置く。未収録: `seed-get-started`（`_meta.json` の死にリンク、sitemap にも無し）、`drizzle-studio/overview`（実体なし）。`gotchas` / `kit-migrations-for-teams` / `kit-web-mobile` は原文スタブ。Hono 連携 guide は公式に存在せず samples 未収録。GA 追跡トリガーは npm dist-tag `latest` が `1.0.0` になった時点で `/update-skill drizzle check`
- `fandhe-vector-db` は fandhe ファミリー 4 件目 — **Rust** 製ローカルファースト vector 特化クエリ DB（PostgreSQL wire protocol v3 互換）。workspace 2 crate（`fandhe-vector-db-engine` lib + `fandhe-vector-db-wire-server` bin）、crates.io **0.1.0**（2026-09-09 公開）を publish commit SHA `7022d112e79760dca916480599553fcac256b5fb` で pin する（tag / release なし、`main` 直参照禁止。fastify v6 alpha / mssql master と同型）。公式ドキュメントサイトは存在せず、出典は README（101KB 日本語・ベンチ手順が大半）+ `docs/design/` ADR 95 件（日本語）+ crate ソース。docs.rs の**レンダリング済み rustdoc ページは 404**（`fandhe-ai` の rendered fetch 前例は適用不可）で source view（`docs.rs/crate/<crate>/0.1.0/source/src/`）のみ取得可、README / ADR は `raw.githubusercontent.com` の pin SHA。**rustdoc（doc comment）は日本語**で書かれており、references は日本語本文 + verbatim 転記（fandhe ファミリー共通）。仕様の source of truth は private リポジトリ `Fandhe-AI/vector-db-spec`（`docs/spec` submodule）で取得不可のため、公開ソース・公開 ADR で追跡できない挙動は書かず、spec 参照 ID（CORE-9 等）のみ残る箇所は「spec 非公開のため未記載」と `## Notes` に明記する（捏造禁止）。ADR はカテゴリ化せず該当ページの Notes 出典としてのみ引用。**WebFetch 要約器は大きな `.rs`（`hnsw.rs` 400KB / `gpu_batch.rs` 241KB 等）で API 表面の捏造・doc comment の誤帰属・variant 欠落を実際に起こした**ため、main が pin SHA の全ソース・ADR を scratchpad に `curl` / `gh api` で保存し、各 researcher が Read の行範囲指定で verbatim 転記する方式に切り替えた（fastify / mssql の共有 DL 方式と同型。`grep -n '^\s*pub'` で pub アイテムを全件列挙してページと突合する）。9 カテゴリ（getting-started / sql / sql-execution / search / storage / security / wire-server / performance / extensions、1 モジュール = 1 ページ、サブディレクトリは親ページに統合）。`description` は「Rust 製ローカルファースト vector 特化クエリ DB fandhe-vector-db (PostgreSQL wire protocol v3 互換) の」で始め、裸の vector / index / search / query / embedding / hybrid / RLS / recall を lead word にしない（`upstash` / `supabase` / `tanstack-query` / `nvidia-cuda` と衝突）。距離線は README には置かず（`mssql` / `drizzle` 先例）per-page `## Notes`: `search/{hnsw,hybrid,sparse,rerank}` → `upstash`、`search/precision` → `openai-evals-tuning` / `anthropic-prompt-eval`、`security/{rls,tenant}` → `supabase` / `drizzle`、`performance/{kernel,isa,gpu-batch,f16,sq8}` → `nvidia-cuda` / `apple-silicon` / `amd-rocm` / `fandhe-ai`、`sql/*` → `mssql` / `drizzle` / `supabase`。双方向は `upstash` の description に 1 行追記のみ。samples は README「wire-server の起動」節・`tests/three_client/` の実クライアントコード（psql / psycopg / node pg）・`compose.yaml` のみから verbatim curate、scripts は Makefile 節と `cargo run -p` に限定（`cargo install` は README 未記載のため未収録、psql 接続コマンドは README に実例が無いため scripts には置かず `samples/connect-*.md` へ案内、bench / CI secrets は除外）。`extensions`（wasm_udf / declarative_filter）は 0.1.0 表面のみ。追跡トリガーは crates.io `fandhe-vector-db-engine` が 0.2.0 になった時点、または docs サイト / `vector-db-spec` 公開時に `/update-skill fandhe-vector-db check`
- `make` は GNU Make / Makefile リファレンス（出典は GNU Make Manual 4.4.1、Edition 0.77。ローカル検証は macOS 同梱の GNU Make 3.81 で行い、3.82+ / 4.x 専用機能は最低バージョンを明記）。`skills/` 配下で唯一、実行可能コードを同梱する拡張スキル — `scripts/bin/*.mjs`（Node 標準ライブラリのみの補助 CLI、既定は静的・dry-run）、`tests/*.test.mjs`（node:test）、`samples/projects/`（Bash / Makefile / Cargo workspace + xtask / pnpm の動くサンプル）、`samples/ci/*.yml.example`・`samples/plans/*.json`。`scripts/` `samples/` の README 索引と `.md` 説明ページの形式は維持し、実ファイルへ案内する。CI は `skill-tests` job の別ステップで `skills/make/tests/**/*.test.mjs` のみを実行（failglob で 0 件は失敗）。references は Source-backed behavior / Design guidance / Example contract を見出しで区別し、`help/doctor/setup/check/fix/verify/clean` の共通化はこの Skill の設計指針であって公式要求ではない。gnu.org は並列 WebFetch で HTTP 429 を返すため、一部ページは docs.w3cub.com ミラー経由で確認し一次出典 URL を併記した。samples/projects の Rust / pnpm サンプルの実ビルドは CI 対象外（ローカル実測のみ）
