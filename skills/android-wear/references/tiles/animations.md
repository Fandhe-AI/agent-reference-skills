...
| `DefaultContentTransitions.fadeIn()` / `.fadeOut()` | `FadeInTransition` / `FadeOutTransition` | Preset fade transitions for `AnimatedVisibility.setEnterTransition`/`setExitTransition`. |
| `DefaultContentTransitions.slideIn(direction)` / `.slideOut(direction)` | `SlideInTransition` / `SlideOutTransition` | Preset slide transitions; `direction` is a `ModifiersBuilders.SLIDE_DIRECTION_*` constant (e.g. `SLIDE_DIRECTION_LEFT_TO_RIGHT`). |
...
- All `Transformation` float/dp properties accept dynamic expressions (`AnimatedFloat`, `DynamicFloat.animate(...)`), so rotation/scale/translation can themselves be animated rather than static.
...
- Package: `androidx.wear.protolayout.expression.AnimationParameters` / `AnimationSpec`, `androidx.wear.protolayout.LayoutElementBuilders.DefaultContentTransitions` / `AnimatedVisibility`, `androidx.wear.protolayout.ModifiersBuilders.Transformation`, `androidx.wear.protolayout.ResourceBuilders.AndroidLottieResourceByResId`.
