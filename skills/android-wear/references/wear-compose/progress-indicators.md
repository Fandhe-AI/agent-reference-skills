...
|------|------|---------|-------------|
| `progress` (Circular/Linear) | `() -> Float` | — | Lambda returning current progress `0f..1f` (or beyond with `allowProgressOverflow`). |
| `allowProgressOverflow` (Circular) | `Boolean` | `false` | Allows progress values > 1 to wrap visually. |
...
- `CircularProgressIndicator` and `LinearProgressIndicator` are determinate (driven by `progress`); `ArcProgressIndicator` shown here is the indeterminate overload (a determinate overload with a `progress` param also exists).
