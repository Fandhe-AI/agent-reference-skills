...
- Troubleshooting: if the SSH check fails, verify network connectivity and SSH access; if the GB10 check fails, remove unsupported hardware (only DGX Spark/GB10 is supported); if the software version check fails, update to April 2026 system software or later; if the password check fails, select "Fix Now" and provide the password for temporary sudo access; if network topology is detected incorrectly, verify cable seating and reboot devices with cables connected; for suboptimal performance, reboot nodes and verify cable types and switch configuration.
