...
- For SQL Server, apply the same encapsulate-and-close pattern with the `mssql` package's `ConnectionPool` (see the separate `mssql` skill); its `Request` class is unrelated to Fastify's own `Request` object.
