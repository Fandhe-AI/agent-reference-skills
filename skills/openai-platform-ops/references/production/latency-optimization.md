...
2. **Generate fewer tokens** — output generation is usually the largest latency contributor; cutting ~50% of output tokens cuts roughly ~50% of latency. Ask for conciseness for natural language; minimize structured-output syntax (short field names, omit named args); use `max_tokens`/`stop` to end generation early.
