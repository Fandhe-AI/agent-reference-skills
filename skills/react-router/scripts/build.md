...
React Router v8 アプリのビルドと各種デプロイ向けコマンド。
...
> **警告**: v8 は React Router の Cloudflare dev proxy（`cloudflareDevProxy`）を削除した。`@cloudflare/vite-plugin` の `cloudflare()` プラグインに置き換える必要がある。Cloudflare Workers デプロイはランタイム制約（Node.js API 非対応）もあるため、サーバーコードの互換性を事前に確認すること。
