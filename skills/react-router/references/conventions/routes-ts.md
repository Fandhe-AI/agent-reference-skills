...
Required configuration file that maps URL patterns to route module files. The default export must satisfy `RouteConfig`. Not available in Data Mode or Declarative Mode.
...
- v8: content confirmed current against `reactrouter.com/api/framework-conventions/routes.ts` (v8.3.0+). No renames/removals.
...
- `relative(directory)` returns `{ route, index, layout }` helpers whose `moduleFile` paths resolve relative to `directory` — useful for splitting `routes.ts` config across multiple files/directories.
