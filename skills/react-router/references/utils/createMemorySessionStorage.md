...
function createMemorySessionStorage<
  Data = SessionData,
  FlashData = Data,
>(options?: MemorySessionStorageOptions): SessionStorage<Data, FlashData>
```

`MemorySessionStorageOptions` accepts an optional `cookie` field (a `CookieOptions & { name?: string }`). Returns an object with `{ getSession, commitSession, destroySession }`.
