...
| --- | --- | --- | --- |
| `routeDiscovery.mode` | `"lazy" \| "initial"` | `"lazy"` | `"lazy"` fetches route metadata progressively; `"initial"` includes all routes in the initial manifest |
| `routeDiscovery.manifestPath` | `string` | `"/__manifest"` | Endpoint path used to fetch manifest patches; customize when running multiple React Router apps on the same domain |
