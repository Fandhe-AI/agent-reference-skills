...
- React Router handles most state needs; reach for Redux/TanStack Query/Apollo only for complex client-only state or advanced caching scenarios
...
- Local storage-based persistence requires manual sync via effects and causes UI flicker on initial load (server can't read `window.localStorage`); cookies avoid this because they are readable during SSR
