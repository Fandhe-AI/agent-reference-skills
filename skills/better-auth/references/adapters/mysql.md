...
- **スキーマ管理**: Better Auth CLI は設定とプラグインに基づいた自動スキーマ生成とマイグレーションを提供。MySQL では両操作が完全にサポートされている。
...
- **データベース Joins（実験的）**: 複数テーブルにまたがる関連データフェッチ時のパフォーマンスを改善し、データベースレイテンシに応じて 2-3 倍のパフォーマンス改善を提供。
