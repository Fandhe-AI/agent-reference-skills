...
重要: Realtime は `realtime.messages` テーブルに実際にはメッセージを保存しない。ポリシーチェックのためだけにクエリを実行し、その後ロールバックする。ストレージではなくアクセス検証の仕組みである。

### プライベートチャネルの設定

プライベートチャネルを有効にするには以下の両方が必要:

1. ダッシュボードの Realtime 設定で「Allow public access」を無効化する
2. クライアントコードでチャネルを `private: true` で作成する
...
アクセスキャッシュ: 認可ポリシーは接続時にキャッシュされ、再接続または新しい JWT 送信時のみ更新される。JWT の有効期限を短く保つことを推奨する。
...
- `realtime.messages` テーブルはアクセス検証のためだけに使用される。実際のメッセージデータは保存されない（データベース経由の Broadcast を除く）
...
- 認可ポリシーのキャッシュは接続時に作成され、再接続または新しい JWT 送信時のみ更新される
