...
約 200 種類の Postgres パフォーマンスシリーズを提供する。現在ベータ版。セルフホスティング環境では利用不可（クラウド専用）。
...
HTTP Basic Auth を使用する。ユーザー名は `service_role`、パスワードは Service Role Key を指定する。

```bash
curl --user "service_role:${SERVICE_ROLE_KEY}" \
  "https://<project-ref>.supabase.co/customer/v1/privileged/metrics"
```

Management API でアクセストークンをプログラム的に交換することも可能。
...
Supabase 公式の Grafana ダッシュボード（200 以上のチャート）を利用可能。
...
```yaml
# Grafana のプロビジョニング設定（HTTP Basic Auth）
apiVersion: 1
datasources:
  - name: Supabase
    type: prometheus
    ...
    secureJsonData:
      basicAuthPassword: "${SERVICE_ROLE_KEY}"
```
