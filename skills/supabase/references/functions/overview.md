...
- **組み込み AI 推論**: `Supabase.ai.Session` API で外部依存なしにテキスト埋め込みや LLM を実行可能

### API キーと認証

Edge Functions では以下のいずれかで認証する:

| キー種別 | 用途 |
|---------|------|
| `sb_publishable_xxx` / anon key | ユーザーの JWT を引き継ぐ RLS 適用アクセス |
| `sb_secret_xxx` / service_role key | RLS バイパスの管理操作 |

新形式キー（`sb_publishable_xxx`）を使用する場合、JWT 検証には `--no-verify-jwt` オプションが必要になる場合がある。
