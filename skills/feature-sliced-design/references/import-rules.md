...
[Steiger](https://github.com/feature-sliced/steiger) は FSD の公式アーキテクチャ linter。インポートルール違反を CI で自動検出する。
...
- 同一フォルダ内の複数ドメインを扱う汎用ファイル（`types.ts`, `utils.ts` 等）が存在する（デセグメント化の兆候）
