...
| --- | --- | --- | --- |
| `root(state, attrs, children)` | `OpenState` | — | ルートコンテナ |
| `trigger`/`anchor`/`positioner`/`arrow`/`arrow_tip`/`close_trigger`/`indicator` | — | — | state 引数以外は headless `popover` と同型の styled 実装（`trigger` は暗黙 submit 事故防止で `type="button"` 固定） |
| `content(state, id, labelledby, describedby, attrs, children)` | `OpenState`, `id: Option<&str>`, `labelledby: Option<&str>`, `describedby: Option<&str>` | すべて `None` | `id`/`labelledby`/`describedby` が `Some` のときそれぞれ `id`/`aria-labelledby`/`aria-describedby` を出力（dialog と異なり `ContentIds` 構造体は使わず個別引数） |
...
- `root`/`trigger`/`content` 等は headless `popover` の同名関数をそのまま再エクスポートしたものではなく、`state`/`id` 等を明示的に取る styled 側固有の実装（dialog/drawer と同じパターン）。`Popover` 状態機械は本モジュールから再エクスポートされる（dialog/drawer とは異なり headless 型を隠さない）
