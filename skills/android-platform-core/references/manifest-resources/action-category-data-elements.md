...
| `android:host` | String | — | URI authority host; meaningless without `scheme`. Supports a leading `*` wildcard for subdomains (`*.example.com`). Case-insensitive. |
...
| `android:pathAdvancedPattern` | String | — | Complete path with regex-like syntax (`.`, `[...]`, `*`, `+`, `{...}`); requires double-escaping in XML. API 31+. |
...
- Multiple `<data>` elements in the same filter are combined by OR; multiple `<action>`/`<category>` elements likewise each independently satisfy their respective test.
