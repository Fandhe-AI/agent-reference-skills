...
|------|------|---------|-------------|
| `PowerManager.newWakeLock(levelAndFlags, tag)` | method | — | Creates (but does not acquire) a `WakeLock`; `tag` should include the package/class/method name for `adb shell dumpsys power` debugging. |
