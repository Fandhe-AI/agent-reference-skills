...
- LLM クエリプランニングのソフトブースト機構（`hybrid.rs`）と RLS 事前フィルタ（`tenant::visible_rows` → `hybrid::hybrid_search_boosted`）の合流点の読み取り検証は ADR [`plan-rls-boost-interaction`](https://raw.githubusercontent.com/Fandhe-AI/vector-db/7022d112e79760dca916480599553fcac256b5fb/docs/design/plan-rls-boost-interaction.md)（対応: TASK-139、Status: Accepted）を参照。`USING PLAN` 実行器（TASK-77）は未実装のため、検証は SQL 表層ではなく engine API 層（`tenant::visible_rows` → `hybrid::path_hint_matches`/`kind_hint_matches` → `hybrid::BoostRule::new` → `hybrid::hybrid_search_boosted`）で行われている。
