...
公開対象は 13 クレート（コア 3 + プラグイン 10）で、すべて v0.1.0（2026-07-21 公開、lockstep）。通常は `fandhe-backend-core` の Cargo feature 経由で利用し、個別プラグインクレートを直接依存に追加する必要はない（`fandhe-backend-plugin-hub-wiring` のみ独立クレートとして直接利用する）。
