...
**新規項目の作成**:
- 新しい references カテゴリ → **reference-researcher（subagent_type: reference-researcher、model: sonnet）**へ委譲。新規カテゴリが複数ある場合は **1 Agent = 1 カテゴリ**で並列起動し、各 Agent に `base_url`（reference-updater が報告した公式ドキュメント URL）、`scope`＝当該カテゴリ、`output_dir`＝`skills/<library>/references/<category>/` を渡す（create-skill Step 3 と同じ規約。reference-researcher は既定 `mode=full` で `base_url` 必須のため必ず渡す）
- **既存カテゴリ内の新規ページ** → 同じく **reference-researcher** へ委譲。reference-updater は `mode=apply` でも新規ページを Write せず推奨アクションに回すため（reference-updater Step 4 mode=apply の規則2）、新規ページが report-only で取り残されないよう、当該カテゴリに対し `base_url`（reference-updater が報告した公式ドキュメント URL）、`scope`＝該当カテゴリ、`output_dir`＝`skills/<library>/references/<category>/` を渡して起動する（既定 `mode=full` で `base_url` 必須のため必ず渡す）。**この委譲では「新規ページのみ作成（既存ファイルは更新しない）」と明示的に指示する**（reference-researcher 行動原則4）。既存ページの内容更新は scoped な reference-updater（`mode=apply`）が既に担当済みのため、reference-researcher には既存ファイルを触らせず不足ページの新規作成だけを行わせ、同一ファイルが両 Agent によって異なる戦略で二重編集されるのを防ぐ
