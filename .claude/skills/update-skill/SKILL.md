...
スコープが広い場合（カテゴリ数が多い等）は scope を分割して reference-updater を並列起動する。ただし scoped 実行は `samples/` `scripts/` を対象にしないため、**scope 分割した場合は Step 2 へ進む前に、scope 未指定（全体）の reference-updater を `mode=check` で1回だけ実行し、`samples/` `scripts/` の差分（および scope 分割で取りこぼしたカテゴリ）を検出する**（この結果を Step 2 の新規/変更追従判定の入力に含める）。
...
各 scoped 実行（apply）と全体実行（check）の「変更カテゴリ一覧」をマージして、後続の readme-indexer 対象と Step 2 の新規/変更追従対象を確定する。
