...
スコープが広い場合（カテゴリ数が多い等）は scope を分割して reference-updater を並列起動する。ただし scoped 実行は `samples/` `scripts/` を対象にしないため、**scope 分割した場合は Step 2 へ進む前に、scope 未指定（全体）の reference-updater を同じ `mode` で必ず1回実行し、`samples/` `scripts/` の差分も検出する**（この結果を Step 2 の新規/変更追従判定の入力に含める）。各 scoped 実行と全体実行の「変更カテゴリ一覧」をマージして後続の readme-indexer 対象を確定する。
