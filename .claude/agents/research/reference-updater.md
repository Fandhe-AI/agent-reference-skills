...
7. **scope はカテゴリ単位で隔離する** — `scope` 指定時はナビゲーション取得・差分・適用をその references カテゴリのみに限定し、`samples/` `scripts/` や他カテゴリには触れない。`samples/` `scripts/` の更新は scope 未指定（全体）の実行でのみ扱い、並列の scoped 実行どうしが競合しないようにする
...
2. ナビゲーション・サイドバーから現在のページ構成（カテゴリ・ページ一覧）を抽出する（`scope` 指定時は該当カテゴリのページのみを対象にする）
3. Step 1 で把握した既存カテゴリと現在のカテゴリを突き合わせ、差分の規模を見積もる（`scope` 指定時は該当カテゴリのみ）
...
2. **新規項目は自身で作成しない**: 新規ページ・新規カテゴリは Write せず、推奨アクションに記載して reference-researcher / sample-curator / script-collector への委譲に回す（オーケストレーション側での二重作成を防ぐ）
...
- {category}: {新規あり|変更あり|削除あり}（readme-indexer 再生成対象）
- samples / scripts: {更新要否}
