...
### Step 3: 末端イシューを worktree 隔離で並列実装する（Implement）
...
### Step 4: push 前のローカル diff を独立レビューする（Review）
...
### Step 4.5: Review 通過後に push + PR を作成する（PR Create）
...
| Review 前に push・PR 作成を行う | push・PR 作成は Review 全通過後の Step 4.5 で行う。Review 失敗時に CI を起動させないための設計 |
| Review fix で push してしまう | Review ループの fix はローカルコミットのみ。push は Step 4.5 のみで行う |
