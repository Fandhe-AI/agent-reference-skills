...
```bash
git log --oneline -- skills/<SKILL_NAME>/
git diff HEAD~1 HEAD -- skills/<SKILL_NAME>/
```
...
対応するパスへローカルの `skills/<SKILL_NAME>/` 配下をコピーします。

```bash
# 例: upstream 側が skills/<name>/ の場合
cp -R "<local-repo>/skills/<SKILL_NAME>/." "$WORKDIR/upstream/skills/<SKILL_NAME>/"
```
