...
# 引数が空の場合: skills/ と .agents/skills/ の両方を列挙してユーザーに選ばせる
# （lockfile 由来のスキルは .agents/skills/ のみに存在する場合がある）

# 対象ディレクトリを解決して LOCAL_SKILL_DIR に設定する
...
後続の Step では `${LOCAL_SKILL_DIR}/` を使ってローカルパスを参照します。
